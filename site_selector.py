...
IMG_HT, IMG_WIDTH = IMG_GRAY.shape  # SHAPE 애트리뷰트를 이용해 이미지의 높이와 너비를 얻기 
PIXELS_PER_KM = IMG_WIDTH / MARS_CIRCUM # 
RECT_WIDTH = int(PIXELS_PER_KM * RECT_HT_KM)
    ...
    def make_final_display(self):
        ...
        for k in self.high_graded_rects:
            ...
            if txt_y >= int(canvas.cget('height')) - 10:
                txt_x += 300
                txt_y = IMG_HT + 20
        canvas.pack() # Canvas상의 객체 위치를 최적화해준다.
        screen.mainloop() # tkinter를 실행하는 무한 루프이다. 창이 닫힐 때까지 루프를 돌면서 이벤트 발생을 기다렸다가 이벤트가 발생하면 처리해준다.
